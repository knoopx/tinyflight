...
FAST_CODE_NOINLINE void updateRcCommands(void)
{
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        rcCommand[axis] = applyRateDynamics(rcCommand[axis], axis, currentRxRefreshRate);
    }
...
FAST_CODE float calculateRateDynamicsK(float k, int time)
{
    if (k == 0.0f) {
        return 0;
    }
...
FAST_CODE float applyRateDynamics(float rcCommand, int axis, int currentRxRefreshRate)
{
    ...
    if (((currentControlRateProfile->rateDynamics.rateSensCenter != 100) || (currentControlRateProfile->rateDynamics.rateSensEnd != 100)) || ((currentControlRateProfile->rateDynamics.rateWeightCenter > 0) || (currentControlRateProfile->rateDynamics.rateWeightEnd > 0))) {
        ...
        iterm_centerStick = inverseRcCommandPercent * rcCommandError * calculateRateDynamicsK(currentControlRateProfile->rateDynamics.rateCorrectionCenter / 100.0f, currentRxRefreshRate); // valid iterm values are between 0-95
        iterm_endStick = rcCommandPercent * rcCommandError * calculateRateDynamicsK(currentControlRateProfile->rateDynamics.rateCorrectionEnd / 100.0f, currentRxRefreshRate);
        iterm[axis] += iterm_centerStick + iterm_endStick;
        rcCommand = rcCommand + iterm[axis]; // add the iterm to the rcCommand

        dterm_centerStick = inverseRcCommandPercent * (lastRcCommandData[axis] - rcCommand) * calculateRateDynamicsK(currentControlRateProfile->rateDynamics.rateWeightCenter / 100.0f, currentRxRefreshRate); // valid dterm values are between 0-95
        dterm_endStick = rcCommandPercent * (lastRcCommandData[axis] - rcCommand) * calculateRateDynamicsK(currentControlRateProfile->rateDynamics.rateWeightEnd / 100.0f, currentRxRefreshRate);
